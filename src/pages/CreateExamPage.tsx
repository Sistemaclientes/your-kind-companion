...
import { storage, Exam, Question } from '@/lib/storage';
...
export function CreateExamPage() {
  ...
  React.useEffect(() => {
    if (id) {
      ...
      if (exam) {
        ...
        setCategory(exam.category || 'Engenharia');
        if (exam.settings) {
  ...
  const handlePublish = async () => {
    ...
    const examData: Exam = {
      ...
      title: title,
      subtitle: `${category} • ${questions.length} questões`,
      status: 'Ativa',
      students: id ? (storage.getExams().find(e => e.id === id)?.students || '0 Alunos') : '0 Alunos',
      date: id ? (storage.getExams().find(e => e.id === id)?.date || new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })) : new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' }),
      questions: questions,
      duration: duration,
      category: category,
      settings: settings

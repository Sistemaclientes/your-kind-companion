...
export function ExamsPage() {
  ...
  const [exams, setExams] = React.useState(storage.getExams());
  ...
  const confirmDeleteExam = () => {
    if (examToDelete) {
      storage.deleteExam(examToDelete);
                    ...
                    <tr key={exam.id} className="hover:bg-primary/[0.02] transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <span className={cn(
                          ...
                          exam.status === 'Ativa' && "bg-green-500/10 text-green-600 dark:text-green-400",
                          exam.status === 'Rascunho' && "bg-orange-500/10 text-orange-600 dark:text-orange-400",
